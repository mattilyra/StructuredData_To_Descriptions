    ...
    with variable_scope.variable_scope(
        ...
        with variable_scope.variable_scope("Encoder_Cell"):
            for i in range(num_fields_g):
                ...
                temp_outputs = array_ops.concat(temp_outputs_list[0], 2)
                temp_outputs = array_ops.unpack(temp_outputs, axis=1)
                field_states.append(array_ops.concat([initial_state_fw, initial_state_bw], 1))
        ...
        encoder_state = array_ops.concat([initial_state_fw, initial_state_bw], 1)
        ...
        for i in range(len(field_states)):
            field_inputs.append(array_ops.concat([field_states[i], embedded_fields[i]], 1))
        # Decoder.
        ...
        field_initial_state = array_ops.concat([field_encoder_fw, field_encoder_bw], 1)

        top_states_field_encoder = [array_ops.reshape(e, [-1, 1, 2*cell_encoder_fw.output_size])
                                    for e in field_outputs]

        attention_states_fields = array_ops.concat(top_states_field_encoder, 1)
