...
class BasicAttention:
    ...
    def inference(self, encoder_inputs1, decoder_inputs1,field_inputs,sequence_length_inputs,  embedding_size, feed_previous,
        ...
        fi = tf.unstack(field_inputs)
        si = tf.unstack(sequence_length_inputs)
        ...
        #self.final_outputs = tf.unstack(self.final_outputs)

        #awts = np.random.randn(46, 3, 75)
        #awts = awts.astype(np.float32)
        #attention_weights_tokens = tf.Variable(awts)
        #attention_weights_tokens = tf.unstack(attention_weights_tokens)
